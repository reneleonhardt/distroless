    ...
    while IFS= read -r arch; do
      ...
      if [ "$verbose" = 1 ]; then
        if [ "$current" = "$version" ]; then
          echo "  ${minor} ${arch}: ${current} (unchanged)" >&2
        else
          echo "  ${minor} ${arch}: ${current:-<none>} -> ${version} (update)" >&2
        fi
      fi
      if [ "$current" != "$version" ]; then
        changed=1
        changes+=("update ${minor} ${arch}: ${current:-<none>} -> ${version}")
      fi
...
"
  ...
  if [ ${#fill[@]} -gt 0 ]; then
    ...
    sed -i "s/^PYTHON_MAJOR_VERSIONS = .*/PYTHON_MAJOR_VERSIONS = [${minors_quoted}]/" "$config_tmp"
    sed -i "s|\"${latest_minor}\": \[[^]]*\],|\"${latest_minor}\": [${arch_list}],${fill_entries}|" "$config_tmp"
    for fill_m2 in "${fill[@]}"; do
    ...
    sed -i "s/^use_repo(py, .*/use_repo(py, ${repos_sorted}, \"python_versions\")/" "$module_tmp"
    for fill_m2 in "${fill[@]}"; do
...
# A broken script must NOT attempt to clean up after itself: its cleanup would be
# as suspect as its failure, and self-restore masks bugs. Instead the updater
# validates everything fallible BEFORE mutating anything (pre-flight checks), so
# a RED run leaves the tree untouched; the only residue to reset is MODULE.bazel.lock
# if a bazel command ran against a mutated tree (git checkout MODULE.bazel.lock).
